...
# curl -o tavern_install.sh https://raw.githubusercontent.com/DarioNonis/Tavern/master/install.sh && sudo bash tavern_install.sh
"echo" "-e" "====================="
"echo" "-e" "Tavern Installer"
"echo" "-e" "====================="
"echo" "-e" "Supported OS: CentOS 7/8, Ubuntu >= 16.04 and Debian"
"echo" "-e" "---------------------------------------"
"echo" "-e" "This script is designed for Linux and basic usage."
"echo" "-e" "For advanced usage, please go to https://github.com/DarioNonis/wiki/Installation"
"echo" "-e" "---------------------------------------"
"echo" "-e" ""
"echo" "-e" "Local - Install Tavern in your current machine with git, Node.js 14 and pm2"
"echo" "-e" "Docker - Install Tavern Docker container"
"echo" "-e" ""
if [ "$1" != "" ]; then
  type="$1"
else
...
if [ "$type" == "local" ]; then
  ...
  npm run setup
  pm2 start server/server.js --name tavern -- --port=$port
else
  defaultVolume="tavern"
  check=$(docker -v)
  if [ "$check" == "" ]; then
  ...
  docker run -d --restart=always -p $port:3001 -v $volume:/app/data --name tavern dariononis/tavern:1
fi
