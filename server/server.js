...
let hostname = process.env.TAVERN_HOST || args.host;
...
const port = parseInt(process.env.TAVERN_PORT || process.env.PORT || args.port || 3001);

// SSL
const sslKey = process.env.TAVERN_SSL_KEY || process.env.SSL_KEY || args["ssl-key"] || undefined;
const sslCert = process.env.TAVERN_SSL_CERT || process.env.SSL_CERT || args["ssl-cert"] || undefined;
const disableFrameSameOrigin = !!process.env.TAVERN_DISABLE_FRAME_SAMEORIGIN || args["disable-frame-sameorigin"] || false;
...
(async () => {
    ...
    app.get("/.well-known/change-password", async (_, response) => {
        response.redirect("https://github.com/DarioNonis/Tavern/wiki/Reset-Password-via-CLI");
    });
        ...
        socket.on("prepare2FA", async (callback) => {
            try {
                ...
                if (user.twofa_status == 0) {
                    ...
                    let uri = `otpauth://totp/Tavern:${user.username}?secret=${encodedSecret}`;
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new Tavern instance, need to setup
    if ((await R.count("user")) === 0) {
...
process.addListener("unhandledRejection", (error, promise) => {
    ...
    console.error("If you keep encountering errors, please report to https://github.com/DarioNonis/Tavern/issues");
});